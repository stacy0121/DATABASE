...
("카페에서 어제 산 커피",1,"플라스틱 컵 씻어서 다시 커피 담아도 되나?ㅋㅋ 세균 번식했으려나",1, 132),
("헐 나 지금 알았는데",2,"해품달이랑 킬미힐이 쓰신 진수완 작가님 우리 학교 선배님이었어..",1, 36),
("내일 시험 끝나고 첫 수업인데",3,"영화 보는거죠? 과자 뭐 챙겨가지",1, 54),
("제시라는 이름에는 무슨 성이 제일 잘 어울려?",4,"김제시, 박제시 뭐 이런 거 있잖아. 아무거나 던져줘봐",1, 54),
("와 나는 진짜 답이 없다.",5,"페트병 버리러 나와서 페트병 말고 핸드폰 버리고 옴. 진짜 한참 찾았네",1, 59),
("집순이의 기준이 뭐임?",2,"슈퍼가 안 가고 집에만 있는거? 아님 동네 아니면 안 돌아다니는거? 아님 친구들 안 만나는거?",1, 46),
("퉁퉁이 엄마 이름 모퉁이래 ㅋㅋㅋㅋㅋㅋ",1,"진짜 웃겨ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ",1, 65),
("나 지금 중국집에서 혼밥 중인데 웃음 참고있어 도와줘",6,"주인 분이 전화받더니 짜장밥에 짜장이 안 갔다구요?함ㅋㅋㅋㅋ",1, 64),
("우리 강아지 오늘 역대급.. 72번 불렀는데 안 옴",3,"날 뭐로 보는 걸까",1, 46),
("연기파배우 실력파가수 진짜 웃기지 않냐ㅋㅋ",4,"요리파 주방장도 나올 기세",1, 31),
("덕우들은",5,"한글 영어 제외하고 개인적으로 가장 매력적인 언어가 뭐라고 생각해?",1, 54),
("다비치 의리 있다",6,"약속 지키네 언니들 사랑해요",1, 65),
("강아지 보다가 버스 놓침",5,"주인분이 어이쿠하며 다음 버스 올 때까지 만지게 해주심 행복하다",1, 32),
("아침에 인문대",3,"몇 시에 문 열어? 8시엔 열리나",1, 65),
("우이천에 분수 나오는 거 알았어?",4,"나도 오늘 알았음",1, 56),
("교정하고",1,"재발하는 경우 있어? 유지장치 잘 껴도",1, 3),
("지금날씨",4,"어떤가요 선생님들..",1, 4),
("오늘의 메뉴",2,"나오면 후기 좀 알려줄 사람",1, 6),
("티빙 더디저트",4,"재밌다",1,6),
("오늘 날씨",6,"어제랑 비슷해?",1, 35),
("자연광에서 찍는 사진이",5,"진심 잘 나온다",1, 4),
("공차 뭐 마실까",4,"추천 좀!!!",1, 26),
("여권발급",1,"보통 얼마나 걸리려나",1, 16),
("도플갱어를 만나면 죽는다잖아..",2,"도플갱어 믿어?",1, 56),
("치킨 주문한지 1시간",1,"지친다ㅎ",1, 16)
;
# 댓글 대댓글 추가
INSERT INTO `database`.`comment`(`writer`,`subject`,`boardFKey`,`parentFKey`) VALUES
(2,"뭐어때 내 눈에 안 보임(지나가던 바이오)",1,0),
(3,"근데 세제로 씻으면 괜찮을듯?(지나가던 화학)",1,0),
(4,"덕우의 마음이 편하면 그걸로 됐어(지나가던 심리)",1,0),
(5,"그냥 새로 사~(지나가던 경영)",1,0),
(6,"안돼~ 쟤 파산하면 어쩌려구(지나가던 국통)",1,4),
(6,"국문과의 자랑!!",2,0),
(3,"우와 대박이다. 킬미힐미 재밌었는데",2,0),(4,"헐 해품달이...? 대박이네",2,0),(5,"드라마화 하신 작가님이실걸? 원작 말고",2,8),(2,"맞아 드라마로 각본하신 거",2,8),
(4,"맛동산이지",3,0),(1,"낼 영화 세얼간이 맞나요?",3,0),(6,"하울의 움직이는 성 아니에요?",3,12),(3,"또 세얼간이야?",3,12), (1,"선",4,0),(6,"ㅋㅋ선제시",4,15),
(3,"ㅋㅋㅋ쓰레기통에 들어가서 찾음?",5,0), (5,"쓰레기통에서 하이 빅스비 외쳤다",5,17),(2,"어디야?하고 물어볼 가치도 없을때",6,0), (6,"그걸 셌어?",9,0), (1,"해외파 외국인",10,0),
(3,"불어! 발음이 고급진 느낌",11,0), (2,"연어...부드러워",11,0), (2,"ㅋㅋㅋ그래도 다행이네",13,0)
;
...
## 3. 가입자들의 성별 나이 별 개수 출력하고, 연령, 성별 순으로 정렬
select gender, (select count(gender) from `user`) as 'count', year(now())-year(`birth`) as age
from `user`
order by age
;
